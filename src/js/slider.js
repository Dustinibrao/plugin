...
const sliderView = document.querySelector(".di-slider--view > ul"),
	sliderViewSlides = document.querySelectorAll(".di-slider--view__slides"),
	arrowLeft = document.querySelector(".di-slider--arrows__left"),
	arrowRight = document.querySelector(".di-slider--arrows__right"),
...
const beforeSliding = (i) => {
	let isActiveItem = document.querySelector(
			".di-slider--view__slides.is-active"
		),
		...
		sliderViewItems = document.querySelector(
			`.di-slider--view__slides:nth-child(${nextItem})`
		);

	// if nextItem is bigger than the # of slides
	if (nextItem > sliderLength) {
		sliderViewItems = document.querySelector(
			".di-slider--view__slides:nth-child(1)"
		);
	}
	...
	if (nextItem == 0) {
		sliderViewItems = document.querySelector(
			`.di-slider--view__slides:nth-child(${sliderLength})`
		);
	}
